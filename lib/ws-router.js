...
export const WSRouter = class extends EventTarget {
  constructor(wsUrl, path) {
    ...
    this.ws = new WebSocket(wsUrl, "ws-router-hub");
    ...
    this.ws.addEventListener("message", ev => {
      ...
      const type = buf[32];
      if (type === 0) {
        const protocols = JSON.parse(new TextDecoder().decode(buf.slice(33)));
        const session = new Session(id, this.ws, key, this, protocols);
        this.sessions.set(key, session);
        const accept = (...args) => session.accept(...args);
        const ev = new WSRouterEvent("connect", {detail: {protocols, accept}});
...
const Session = class {
  ...
  accept(protocol = null) {
    if (this.accepted) throw Error("already accepted");
    const typeCode = new Uint8Array([0]);
    const buf = new TextEncoder().encode(protocol || "");
